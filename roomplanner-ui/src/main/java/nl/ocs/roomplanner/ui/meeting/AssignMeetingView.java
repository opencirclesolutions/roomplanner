...
public class AssignMeetingView extends BaseView {
    ...
    public void enter(ViewChangeEvent event) {
        ...
        roomMap = locations.stream().collect(Collectors.toMap(Function.identity(),
    ...
    private InMemoryTreeTable<Integer, Room, Integer, Location> createAssignTree(final List<Meeting> meetings,
        ...
        final List<LocalDate> dates = meetings.stream().map(m -> m.getMeetingDate()).distinct().sorted()
                        ...
                        if (location.isPresent()) {
                            ...
                            if (theRoom.isPresent()) {
                                ...
                                if (theMeeting.isPresent()) {
                                    return getMeetingErrorString(theMeeting.get(), checkEmployeeDoubleBooking,
                                            meetings);
                                }
                            }
                        }
        ...
        solveButton.addClickListener(e -> {
            ...
            SimpleModalDialog dialog = new SimpleModalDialog(false) {
                ...
                protected void doBuild(Layout parent) {
                    ...
                    builder.append(
                            meetings.stream().map(m -> getMeetingErrorString(m, settings.isCheckEmployeeDoubleBooking(),
                    ...
                    builder.append(String.format("Best score was: %d HARD, %d MEDIUM, %d SOFT",
                            solution.getScore().getHardScore(), solution.getScore().getMediumScore(),
                            solution.getScore().getSoftScore()));
    ...
    private String getMeetingErrorString(Meeting m, boolean checkEmployeeDoubleBooking, List<Meeting> allMeetings) {
        ...
        if (m.getVideoConferencing() && !m.getRoom().getVideoConferencing()) {
            builder.append(
                    String.format("No room with video conferencing equipment available for %s\n", m.getDescription()));
        }

        if (m.getPhoneConferencing() && !m.getRoom().getPhoneConferencing()) {
            builder.append(
                    String.format("No room with audio conferencing equipment available for %s\n", m.getDescription()));
        }

        if (m.getAttendees().size() > m.getRoom().getCapacity()) {
            builder.append(String.format("Not enough capacity (%d needed, %d available) for meeting %s",
                    m.getAttendees().size(), m.getRoom().getCapacity(), m.getDescription()));
        }
